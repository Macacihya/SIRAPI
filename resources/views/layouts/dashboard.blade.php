{{-- ============================================================
     ...
     Deskripsi: Master layout untuk halaman Dashboard umum,
     Data Guru, Data Siswa, dan Data Sekolah.
     Di-extract dari sidebar yang sebelumnya di-copy-paste
     di setiap halaman (dashboard, guru/index, siswa).
...
<body class="bg-gray-100 min-h-screen flex">
    ...
    <aside class="w-56 bg-white border-r border-gray-200 flex flex-col min-h-screen fixed top-0 left-0">
        ...
        <nav class="flex-1 px-2 py-4 space-y-0.5">
            ...
            @php $currentActive = View::yieldContent('active', 'dashboard'); @endphp

            <a href="{{ route('dashboard') }}"
                class="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm {{ $currentActive === 'dashboard' ? 'font-medium bg-gray-100 text-gray-900' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-800 transition-colors' }}">
            ...
            <a href="{{ route('sekolah.index') }}"
                class="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm {{ $currentActive === 'sekolah' ? 'font-medium bg-gray-100 text-gray-900' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-800 transition-colors' }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('guru.index') }}"
                class="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm {{ $currentActive === 'guru' ? 'font-medium bg-gray-100 text-gray-900' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-800 transition-colors' }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('siswa.tampilkan') }}"
                class="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm {{ $currentActive === 'siswa' ? 'font-medium bg-gray-100 text-gray-900' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-800 transition-colors' }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
