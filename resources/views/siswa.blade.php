...
<head>
    ...
    <title>Data Siswa - SIRAPI</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700&display=swap" rel="stylesheet">
    <style>
        * { font-family: 'Plus Jakarta Sans', sans-serif; }
        .status-aktif  { background:#ecfdf5; color:#059669; border:1px solid #a7f3d0; }
        .status-leave  { background:#fff7ed; color:#ea580c; border:1px solid #fed7aa; }
        .status-cuti   { background:#eff6ff; color:#2563eb; border:1px solid #bfdbfe; }
        .row-check:checked ~ td { background:#f8fafc; }
...
<body class="bg-gray-100 min-h-screen flex">
    ...
    <aside class="w-56 bg-white border-r border-gray-200 flex flex-col min-h-screen fixed top-0 left-0 z-20">
        ...
        <nav class="flex-1 px-2 py-4 space-y-0.5">
            ...
            <a href="{{ route('siswa.tampilkan') }}" class="sidebar-link active">
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path d="M17 20h5v-2a4 4 0 00-5-3.87M9 20H4v-2a4 4 0 015-3.87m6-4a4 4 0 11-8 0 4 4 0 018 0zm6-4a2 2 0 11-4 0 2 2 0 014 0zM5 8a2 2 0 11-4 0 2 2 0 014 0z" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
                Data Siswa
        ...
        <main class="flex-1 p-8 fade-in">
            ...
            <div class="flex items-start justify-between mb-7">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">Pengelolaan data induk siswa, verifikasi NISN, dan monitoring status aktif akademik.</p>
                </div>
                <div class="flex items-center gap-2 flex-shrink-0">
                    ...
                    <a href="#" class="flex items-center gap-1.5 px-4 py-2.5 rounded-lg text-sm font-medium text-white bg-gray-900 hover:bg-gray-800 transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @php
                ...
                $siswaAktif   = collect($data)->where('status', 'Aktif')->count() ?: $totalSiswa;
                $statusCuti   = collect($data)->where('status', 'Cuti')->count()  ?: 0;
                $mengundurkan = collect($data)->where('status', 'Keluar')->count() ?: 0;
            @endphp

            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                <div class="bg-white border border-gray-200 rounded-xl p-5">
                    <p class="text-[11px] font-semibold uppercase tracking-wider text-gray-400 mb-3">Total Siswa</p>
                    <p class="text-3xl font-bold text-gray-900">{{ number_format($totalSiswa) }}</p>
                </div>
                <div class="bg-white border border-gray-200 rounded-xl p-5">
                    <p class="text-[11px] font-semibold uppercase tracking-wider text-gray-400 mb-3">Siswa Aktif</p>
                    <p class="text-3xl font-bold text-gray-900">{{ number_format($siswaAktif) }}</p>
                </div>
                <div class="bg-white border border-gray-200 rounded-xl p-5">
                    <p class="text-[11px] font-semibold uppercase tracking-wider text-gray-400 mb-3">Status Cuti</p>
                    <p class="text-3xl font-bold text-gray-900">{{ $statusCuti }}</p>
                </div>
                <div class="bg-white border border-gray-200 rounded-xl p-5">
                    <p class="text-[11px] font-semibold uppercase tracking-wider text-gray-400 mb-3">Mengundurkan Diri</p>
                    <p class="text-3xl font-bold text-gray-900">{{ $mengundurkan }}</p>
                </div>
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-xl overflow-hidden mb-6">
                ...
                <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between gap-4 flex-wrap">
                    <div class="flex items-center gap-2">
                        ...
                        <div class="relative">
                            <select class="appearance-none pl-9 pr-8 py-2 border border-gray-200 rounded-lg text-sm text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-gray-200 cursor-pointer">
                                ...
                                @foreach(array_unique(array_column($data, 'kelas')) as $kelas)
                                    <option>{{ $kelas }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="relative">
                            ...
                            <svg class="w-4 h-4 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <circle cx="12" cy="12" r="3" stroke-width="2"/>
                                <path d="M12 2v2m0 16v2M4.22 4.22l1.42 1.42m12.72 12.72l1.42 1.42M2 12h2m16 0h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42" stroke-width="2" stroke-linecap="round"/>
                            </svg>
                            <svg class="w-3.5 h-3.5 text-gray-400 absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path d="M19 9l-7 7-7-7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                    ...
                    <p class="text-xs text-gray-400">Menampilkan 1–{{ count($data) }} dari {{ count($data) }} siswa</p>
                </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr>
                            <th class="px-6 py-3 text-left w-10">
                                <input type="checkbox" class="rounded border-gray-300 text-gray-800 focus:ring-0 cursor-pointer">
                            </th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors group">
                            ...
                            <td class="px-4 py-3.5 text-gray-500 text-xs font-mono">{{ $siswa['nis'] }}</td>

                            <td class="px-4 py-3.5 font-medium text-gray-800">{{ $siswa['kelas'] }}</td>

                            {{-- Jenis Kelamin - derived from name heuristic or default --}}
                            <td class="px-4 py-3.5 text-gray-600">
                                {{ isset($siswa['jenis_kelamin']) ? $siswa['jenis_kelamin'] : 'Laki-laki' }}
                            </td>

                            {{-- Status --}}
                            <td class="px-4 py-3.5">
                                @php $status = $siswa['status'] ?? 'Aktif'; @endphp
                                @if($status === 'Aktif')
                                    <span class="status-aktif text-xs font-semibold px-2.5 py-1 rounded-md">AKTIF</span>
                                @elseif($status === 'Cuti')
                                    <span class="status-cuti text-xs font-semibold px-2.5 py-1 rounded-md">CUTI</span>
                                @else
                                    <span class="status-leave text-xs font-semibold px-2.5 py-1 rounded-md">LEAVE</span>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-3.5">
                                <div class="relative" x-data="{ open: false }">
                                    ...
                                    <div class="dropdown-menu hidden absolute right-0 mt-1 w-36 bg-white border border-gray-200 rounded-lg shadow-lg z-10 overflow-hidden">
                ...
                <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        <button class="w-8 h-8 rounded-lg text-gray-500 hover:bg-gray-100 text-sm transition-colors">125</button>
                        <button class="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 transition-colors">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div class="bg-white border border-gray-200 rounded-xl p-6">
                    <h3 class="text-sm font-semibold text-gray-800 mb-2">Verifikasi Data Siswa</h3>
                    <p class="text-xs text-gray-500 leading-relaxed">
                        Pastikan semua NISN sudah terverifikasi dengan pangkalan data kemdikbud (DAPODIK).
                        Data yang belum terverifikasi akan ditandai dengan ikon peringatan pada kolom aksi.
                    </p>
                </div>
                ...
                <div class="bg-white border border-gray-200 rounded-xl p-6">
                    ...
                    <h3 class="text-sm font-semibold text-gray-800 mb-1">Analisis Pertumbuhan</h3>
                    <p class="text-xs text-gray-500 leading-relaxed">
                        Peningkatan pendaftaran tahun ini mencapai 12% dibandingkan tahun ajaran 2022/2023.
                    </p>
                </div>
                ...
                <div class="bg-white border border-gray-200 rounded-xl p-6">
                    ...
                    <p class="text-xs text-gray-500 leading-relaxed">
                        Setiap perubahan data siswa dicatat dalam log sistem untuk audit administrasi.
                    </p>
                </div>
            </div>
