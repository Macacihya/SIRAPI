{{-- Halaman: profil-kelas --- menggunakan layout walikelas --}}
...
>
    <div class="space-y-8" x-data="{
        ...
        <div class="space-y-4">
            ...
            <div class="flex flex-col items-center justify-between gap-3 sm:flex-row">
                ...
                <div class="flex items-center gap-1">
                    <button class="flex h-8 w-8 items-center justify-center rounded-lg text-[#94a3b8] transition hover:bg-[#f1f5f9]">â€¹</button>
                    ...
                    <button class="flex h-8 w-8 items-center justify-center rounded-lg text-[#64748b] transition hover:bg-[#f1f5f9]">â€º</button>
                </div>
            </div>
        </div>

        {{-- â•â•â•â•â•â•â• MODAL EDIT SISWA â•â•â•â•â•â•â• --}}
                    ...
                    <div>
                        ...
                        <select x-model="editForm.role" class="h-12 w-full rounded-lg border border-[#e2e8f0] bg-white px-4 text-[14px] font-semibold text-[#0f172a] outline-none transition focus:border-[#3b82f6] focus:ring-2 focus:ring-[#3b82f6]/20">
                            <template x-for="role in ['SISWA', 'KETUA KELAS', 'WAKIL KETUA', 'SEKRETARIS 1', 'SEKRETARIS 2', 'BENDAHARA 1', 'BENDAHARA 2']" :key="role">
                                <option :value="role" :disabled="!isRoleAvailable(role) && editForm.role !== role" x-text="role + (limits[role] < 999 ? (isRoleAvailable(role) || editForm.role === role ? ' â€” Tersedia' : ' â€” Terisi') : '')"></option>
                            </template>
                        </select>
                    </div>
        ...
        {{-- â•â•â•â•â•â•â• MODAL KONFIRMASI SIMPAN â•â•â•â•â•â•â• --}}
