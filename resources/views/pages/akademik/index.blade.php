...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('akademikData', () => ({
            ...
            periodeAktif: { tahun: '2025/2026', semester: 'Genap', mulai: '2026-01-08', selesai: '2026-06-21' },
            kelasForm: { nama: '', kapasitas: 32, wali: '', jurusan: 'Umum' },
            daftarGuru: [
            ...
            kelas: [
                { id:1, nama:'1-A', jurusan:'Umum', kapasitas:'32 / 32', max:32, pct:100, wali:'Wali Kelas: Drs. Budi Santoso', siswa:['Ahmad F.','Siti A.','Budi D.','Rina M.'] },
                { id:2, nama:'1-B', jurusan:'Umum', kapasitas:'28 / 32', max:32, pct:88, wali:'Wali Kelas: Rina Permata, M.Si.', siswa:['Dimas P.','Dewi K.','Andi W.'] },
                { id:3, nama:'2-A', jurusan:'Umum', kapasitas:'30 / 32', max:32, pct:94, wali:'Wali Kelas: Siti Aminah, M.Pd', siswa:['Rizky P.','Bella N.'] },
            ],
            antrean: [
                {init:'AD',name:'Aditya Pratama',nisn:'0092182711',gender:'Laki-laki',peminatan:'Umum'},
                {init:'BP',name:'Bella Puspita',nisn:'0092182744',gender:'Perempuan',peminatan:'Umum'},
                {init:'DR',name:'Deni Ramadhan',nisn:'0092182788',gender:'Laki-laki',peminatan:'Umum'},
                {init:'FH',name:'Fitri Handayani',nisn:'0092182799',gender:'Perempuan',peminatan:'Umum'},
                {init:'GS',name:'Galih Saputra',nisn:'0092182801',gender:'Laki-laki',peminatan:'Umum'},
            ],
            init() {
                const saved = JSON.parse(localStorage.getItem('sirapi-akademik') || 'null');
                if (saved) {
                    this.periodeAktif = saved.periodeAktif || this.periodeAktif;
                    this.kelas = saved.kelas || this.kelas;
                    this.antrean = saved.antrean || this.antrean;
                }
            },
            persist() {
                localStorage.setItem('sirapi-akademik', JSON.stringify({
            ...
            savePeriode() {
                this.periodeAktif = {...this.periodeForm};
                this.showUbahPeriode = false;
                this.persist();
                this.$dispatch('toast',{message:'Periode akademik berhasil diperbarui!',type:'success'});
            },
            addKelas() {
                if (!this.kelasForm.nama || !this.kelasForm.wali) {
                    this.$dispatch('toast',{message:'Lengkapi nama kelas dan wali kelas.',type:'error'});
                    return;
                }
                const max = Number(this.kelasForm.kapasitas) || 32;
                this.kelas.push({id:Date.now(),nama:this.kelasForm.nama,jurusan:this.kelasForm.jurusan || 'Umum',kapasitas:'0 / '+max,max,pct:0,wali:'Wali Kelas: '+this.kelasForm.wali,siswa:[]});
                this.kelasForm={nama:'',kapasitas:32,wali:'',jurusan:'Umum'};
            ...
            runPlotting() {
                let placed = 0;
                this.antrean = this.antrean.filter((s) => {
                    const target = this.kelas.find(k => (k.siswa || []).length < (k.max || 32));
                    if (!target) return true;
                    target.siswa.push(s.name);
                    target.kapasitas = `${target.siswa.length} / ${target.max || 32}`;
                    target.pct = Math.round((target.siswa.length / (target.max || 32)) * 100);
                    placed++;
                    return false;
                });
                this.showPlotting = false;
                this.persist();
                this.$dispatch('toast',{message: placed ? `${placed} siswa berhasil diplotting.` : 'Tidak ada kapasitas kelas yang tersedia.', type: placed ? 'success' : 'error'});
            },
...
<div x-data="akademikData" class="space-y-6">
    ...
    <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_280px]">
        <div class="space-y-5">
            <div class="flex items-start justify-between">
                <div><h1 class="text-[28px] font-black tracking-[-0.04em] text-[#0f172a]" x-text="'Tahun Ajaran ' + periodeAktif.tahun"></h1><p class="mt-1 text-[14px] text-[#475569]">Linimasa kegiatan akademik berjalan.</p></div>
    ...
    <div>
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <button @click="showTambahKelas = true" class="flex h-[38px] items-center gap-2 rounded-[8px] border border-[#e2e8f0] bg-white px-4 text-[12px] font-bold text-[#334155] transition hover:bg-[#f1f5f9]"><svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 4v16m8-8H4" stroke-width="2" stroke-linecap="round"></path></svg>Tambah Kelas</button>
                <button @click="showPlotting = true" :disabled="antrean.length === 0" class="flex h-[38px] items-center gap-2 rounded-[8px] bg-[#1d4ed8] px-4 text-[12px] font-bold text-white transition hover:bg-[#1e40af] disabled:cursor-not-allowed disabled:opacity-50"><svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 4v16m8-8H4" stroke-width="2" stroke-linecap="round"></path></svg>Plotting Otomatis</button>
            </div>
        </div>
        <div class="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <template x-for="k in kelas" :key="k.id">
                <div @click="openDetail(k)" class="cursor-pointer rounded-[14px] border border-[#e2e8f0] bg-white p-5 transition hover:shadow-md hover:border-[#3b82f6]">
                    ...
                    <p class="text-[11px] font-semibold text-[#64748b]" x-text="'Tahun Ajaran ' + periodeAktif.tahun"></p>
    ...
    <div class="rounded-[14px] border border-[#e2e8f0] bg-white overflow-hidden">
        ...
        <table class="w-full text-[13px]">
            <thead><tr class="border-b border-[#e2e8f0] bg-[#f8fafc]"><th class="px-6 py-3 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-[#64748b]">No</th><th class="px-6 py-3 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-[#64748b]">Nama</th><th class="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-[#64748b]">NISN</th><th class="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-[#64748b]">Gender</th><th class="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-[#64748b]">Peminatan</th></tr></thead>
            <tbody>
                <template x-for="(s, index) in antrean.slice(0,3)" :key="s.nisn">
        ...
        <div class="border-t border-[#e2e8f0] px-6 py-3 text-center">
            <button @click="showAntrean = true" class="text-[12px] font-bold uppercase tracking-[0.08em] text-[#1d4ed8] transition hover:text-[#1e40af]" x-text="antrean.length > 3 ? 'Lihat Semua Antrean (' + (antrean.length - 3) + ' Lainnya)' : 'Lihat Semua Antrean'"></button>
        </div>
    </div>
    ...
    <x-modal alpineShow="showUbahPeriode" title="Ubah Periode Akademik" maxWidth="md">
        ...
        <x-slot:footer>
            ...
            <button @click="savePeriode()" class="flex-1 rounded-lg bg-[#1d4ed8] py-2.5 text-[12px] font-bold text-white">Simpan</button>
        </x-slot:footer>
    </x-modal>
    ...
    <x-modal alpineShow="showAntrean" title="Semua Antrean Plotting" maxWidth="2xl">
        <div class="max-h-[60vh] overflow-y-auto -mx-6">
            <table class="w-full text-[13px]">
                <thead><tr class="border-b border-[#e2e8f0] bg-[#f8fafc]"><th class="px-6 py-3 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-[#64748b]">No</th><th class="px-6 py-3 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-[#64748b]">Nama</th><th class="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-[#64748b]">NISN</th><th class="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-[#64748b]">Gender</th><th class="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-[#64748b]">Peminatan</th></tr></thead>
                <tbody><template x-for="(s, index) in antrean" :key="s.nisn"><tr class="border-b border-[#f1f5f9] transition hover:bg-[#f8fafc]"><td class="px-6 py-3 font-semibold text-[#64748b]" x-text="index + 1"></td><td class="px-6 py-3"><div class="flex items-center gap-3"><div class="flex h-8 w-8 flex-none items-center justify-center rounded-[6px] bg-[#eff6ff] text-[10px] font-bold text-[#1d4ed8]" x-text="s.init"></div><span class="font-bold text-[#0f172a]" x-text="s.name"></span></div></td><td class="px-4 py-3 font-mono text-[12px] text-[#64748b]" x-text="s.nisn"></td><td class="px-4 py-3 text-[#475569]" x-text="s.gender"></td><td class="px-4 py-3"><span class="rounded bg-[#1d4ed8] px-2 py-0.5 text-[10px] font-bold text-white" x-text="s.peminatan"></span></td></tr></template></tbody>
            </table>
        </div>
    </x-modal>
