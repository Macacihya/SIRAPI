...
<div class="space-y-6 pb-24" x-data="capaianData()">

    {{-- Form for batch submission --}}
    <form id="capaianForm" method="POST" action="{{ route('capaian-kompetensi.store-batch') }}" class="hidden">
        @csrf
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div class="flex gap-2">
            <select x-model="filterKelas" @change="applyFilter()" class="h-11 rounded-lg border border-[#e2e8f0] bg-white px-4 text-[13px] font-bold text-[#0f172a] outline-none transition focus:border-[#3b82f6] focus:ring-2 focus:ring-[#3b82f6]/20">
                @foreach($kelasList as $kelas)
                    <option value="{{ $kelas->id }}" {{ $selectedKelasId == $kelas->id ? 'selected' : '' }}>{{ $kelas->nama_kelas }}</option>
                @endforeach
            </select>
            <select x-model="filterMapel" @change="applyFilter()" class="h-11 rounded-lg border border-[#e2e8f0] bg-white px-4 text-[13px] font-bold text-[#0f172a] outline-none transition focus:border-[#3b82f6] focus:ring-2 focus:ring-[#3b82f6]/20">
                @foreach($mapelList as $mapel)
                    <option value="{{ $mapel->kode_mapel }}" {{ $selectedMapelId == $mapel->kode_mapel ? 'selected' : '' }}>{{ $mapel->nama_mapel }}</option>
                @endforeach
            </select>
        </div>
    </div>
        ...
        <div class="overflow-y-auto max-h-[600px] divide-y divide-[#e2e8f0]/80">
                ...
                <template x-for="(s, index) in filteredStudents" :key="s.id">
                    <div class="flex flex-col lg:flex-row gap-4 lg:gap-6 p-5 lg:p-6 transition hover:bg-[#f8fafc]">
                        ...
                        <div class="flex-1 relative flex flex-col justify-center">
                            <template x-if="!s.nilai_id">
                                <div class="rounded-lg bg-[#fef2f2] p-4 text-[13px] text-[#dc2626] border border-[#fecaca]">
                                    <strong>Perhatian:</strong> Siswa ini belum memiliki data nilai di mapel ini. Harap isi nilai siswa di menu Penilaian terlebih dahulu sebelum mengisi capaian kompetensi.
                                </div>
                            </template>
                            <template x-if="s.nilai_id">
                                <textarea 
            ...
            <template x-if="filteredStudents.length === 0">
                <div class="flex flex-col items-center justify-center py-16">
                    ...
                    <p class="mt-3 text-[14px] font-bold text-[#64748b]">Tidak ada siswa yang ditemukan.</p>
                </div>
            </template>
        </div>
    </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('capaianData', () => ({
            ...
            get filledCount() {
                return this.filteredStudents.filter(s => s.capaian && s.capaian.trim() !== '').length;
            },

            applyFilter() {
                window.location.href = `{{ route('capaian-kompetensi') }}?kelas_id=${this.filterKelas}&mapel_id=${this.filterMapel}`;
            },

            applyGlobalToEmpty() {
                if(!this.globalText.trim()) return;
                this.filteredStudents.forEach(s => {
                    if(s.status !== 'Final' && s.nilai_id && (!s.capaian || !s.capaian.trim())) {
            ...
            submitForm(action) {
                ...
                // Clear existing inputs except token and action
                Array.from(form.children).forEach(child => {
                    if(child.name !== '_token' && child.name !== 'action') {
                ...
                this.students.forEach((s, i) => {
                    if(s.nilai_id) {
                        const inputNilaiId = document.createElement('input');
                        inputNilaiId.type = 'hidden';
                        inputNilaiId.name = `capaian[${i}][nilai_id]`;
                        inputNilaiId.value = s.nilai_id;
                        form.appendChild(inputNilaiId);

                        const inputDeskripsi = document.createElement('input');
                        inputDeskripsi.type = 'hidden';
                        inputDeskripsi.name = `capaian[${i}][deskripsi]`;
                        inputDeskripsi.value = s.capaian || '';
                        form.appendChild(inputDeskripsi);
                    }
                });
