...
<div class="space-y-8" x-data="{
    ...
    get filteredSiswa() {
        ...
        if (this.searchQuery) { const q = this.searchQuery.toLowerCase(); result = result.filter(s => s.nama.toLowerCase().includes(q) || s.nis.toLowerCase().includes(q)); }
    ...
    get totalPages() { return Math.ceil(this.filteredSiswa.length / this.perPage) || 1; },
    get paginatedSiswa() { const start = (this.currentPage - 1) * this.perPage; return this.filteredSiswa.slice(start, start + this.perPage); },
    ...
    resetFilters() { this.searchQuery = ''; this.filterJK = ''; this.currentPage = 1; this.filterOpen = false; },
    exportCSV() {
        let csv = 'No,NIS / NISN,Nama Siswa,Jenis Kelamin,Kelas\n';
        this.filteredSiswa.forEach(s => { csv += s.displayNo + ',' + s.nis + ',' + s.nama + ',' + s.jk + ',' + s.kelas + '\n'; });
                ...
                <tbody>
                    <template x-for="s in paginatedSiswa" :key="s._idx">
                        <tr class="border-b border-[#f1f5f9] transition hover:bg-[#f8fafc]">
                            ...
                            <td class="px-5 py-4"><div class="flex items-center gap-3"><div class="flex h-8 w-8 items-center justify-center rounded-md bg-[#1e40af] text-[10px] font-bold text-white" x-text="s.init"></div><span class="font-bold text-[#0f172a]" x-text="s.nama"></span></div></td>
