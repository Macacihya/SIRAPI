...
<head>
    ...
    <style>
        ...
        * { font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        .status-cuti  { background: var(--panel-bg); color: var(--brand-2); border:1px solid var(--field-border); }
        ...
        /* Dropdown */
        .dropdown-menu { position:absolute; right:0; margin-top:4px; width:144px; background:#fff; border:1px solid var(--field-border); border-radius:10px; box-shadow:0 8px 20px rgba(15,23,42,.1); z-index:10; overflow:hidden; }
    ...
    <div class="ml-56 flex-1 flex flex-col min-h-screen">
        ...
        <main class="flex-1 p-8 fade-in">
            ...
            <div class="flex items-start justify-between mb-7">
                <div>
                    <h1 class="text-2xl font-black tracking-tight" style="color:var(--brand-3)">Data Siswa</h1>
                    <p class="text-sm mt-1" style="color:var(--text-soft)">Pengelolaan data induk siswa, verifikasi NISN, dan monitoring status aktif akademik.</p>
                </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                <div class="stat-card p-5">
                    <p class="stat-label mb-3">Total Siswa</p>
                    <p class="stat-value">{{ number_format($totalSiswa) }}</p>
                </div>
                <div class="stat-card p-5">
                    <p class="stat-label mb-3">Siswa Aktif</p>
                    <p class="stat-value">{{ number_format($siswaAktif) }}</p>
                </div>
                <div class="stat-card p-5">
                    <p class="stat-label mb-3">Status Cuti</p>
                    <p class="stat-value">{{ $statusCuti }}</p>
                </div>
                <div class="stat-card p-5">
                    <p class="stat-label mb-3">Mengundurkan Diri</p>
                    <p class="stat-value">{{ $mengundurkan }}</p>
                </div>
            </div>
            ...
            <div class="table-card overflow-hidden mb-6">
                ...
                <div class="toolbar-border px-6 py-4 flex items-center justify-between gap-4 flex-wrap">
                    ...
                    <p class="text-xs" style="color:var(--text-muted)">Menampilkan 1–{{ count($data) }} dari {{ count($data) }} siswa</p>
                </div>
                    ...
                    <thead>
                        <tr>
                            <th class="px-6 py-3 text-left w-10">
                                <input type="checkbox" class="rounded border-gray-300 focus:ring-0 cursor-pointer">
                            </th>
                        ...
                        <tr class="row-divider transition-colors">
                            ...
                            <td class="px-4 py-3.5">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="font-semibold" style="color:var(--brand-3)">{{ $siswa['nama'] }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-3.5 text-xs font-mono" style="color:var(--text-muted)">{{ $siswa['nis'] }}</td>
                            <td class="px-4 py-3.5 font-semibold" style="color:var(--brand-3)">{{ $siswa['kelas'] }}</td>
                            <td class="px-4 py-3.5" style="color:var(--text-soft)">
                                {{ $siswa['jenis_kelamin'] ?? 'Laki-laki' }}
                            </td>
                            <td class="px-4 py-3.5">
                                @php $status = $siswa['status'] ?? 'Aktif'; @endphp
                                @if($status === 'Aktif')
                                    <span class="status-aktif text-xs font-bold px-2.5 py-1 rounded-md">AKTIF</span>
                                @elseif($status === 'Cuti')
                                    <span class="status-cuti text-xs font-bold px-2.5 py-1 rounded-md">CUTI</span>
                                @else
                                    <span class="status-leave text-xs font-bold px-2.5 py-1 rounded-md">LEAVE</span>
                                @endif
                            </td>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div class="info-card p-6">
                    <h3 class="text-sm font-bold mb-2" style="color:var(--brand-3)">Verifikasi Data Siswa</h3>
                    <p class="text-xs leading-relaxed" style="color:var(--text-soft)">
                        Pastikan semua NISN sudah terverifikasi dengan pangkalan data kemdikbud (DAPODIK). Data yang belum terverifikasi akan ditandai dengan ikon peringatan pada kolom aksi.
                    </p>
                </div>
                <div class="info-card p-6">
                    ...
                    <h3 class="text-sm font-bold mb-1" style="color:var(--brand-3)">Analisis Pertumbuhan</h3>
                    <p class="text-xs leading-relaxed" style="color:var(--text-soft)">
